...
serve(async (req) => {
  ...
  try {
    ...
    // Hardened check: Never proceed without a userId
    if (!userId) {
      console.error(`[GLOBEGENIE_LOG] [${requestId}] ⚠️ userId missing from request!`);
      throw new Error("User authentication context is required to generate and save a trip.");
    }
    ...
    // 2. ENRICHMENT PHASE REMOVED FROM MAIN LOOP (Phase 2 will handle this)
    // We strictly use placeholders for now to guarantee 60s safety.
    ...
    itinerary.forEach((day: any, dIdx: number) => {
      ...
      day.activities?.forEach((act: any) => {
        ...
        act.youtubeVideos = []; // Will be enriched in Step 2
        act.rating = 4.8;
      });
    });

    await logEvent(supabase, requestId, userId, 'CURATION_COMPLETE', 'success', `AI curation finished, entering Phase 1 persistence`);

    // 3. Persist Trip to Database
    console.log(`[GLOBEGENIE_LOG] [${requestId}] 💾 Persisting itinerary to database for user: ${userId}`);
    
    // Explicitly verify Supabase secrets
    if (!supabaseUrl || !supabaseServiceKey) {
      console.error(`[GLOBEGENIE_LOG] [${requestId}] 🚨 Supabase Auto-Secrets Missing!`);
      throw new Error("Internal Persistence Error: Missing DB credentials.");
    }
    ...
    if (tripErr) {
      ...
      throw new Error(`Database Error (Trip): ${tripErr.message}`);
    }
    ...
    if (daysErr) {
      ...
      throw new Error(`Database Error (Days): ${daysErr.message}`);
    }

    if (!days) throw new Error("Failed to create itinerary days.");
    ...
    if (actsToInsert.length > 0) {
      ...
      if (actsErr) {
        ...
        throw new Error(`Database Error (Activities): ${actsErr.message}`);
      }
    }
