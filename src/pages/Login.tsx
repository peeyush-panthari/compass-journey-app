    ...
    <div className="min-h-[100svh] bg-hero-gradient flex items-center justify-center p-4 relative overflow-hidden safe-top safe-bottom">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        <div className="bg-card rounded-2xl shadow-elevated p-6 sm:p-8 border border-border/60">
          ...
          <Button variant="outline" className="w-full h-12 mb-6 font-medium rounded-xl" onClick={handleGoogleLogin}>
          ...
          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border" /></div><div className="relative flex justify-center"><span className="bg-card px-3 text-xs text-muted-foreground">or sign in with mobile</span></div></div>
          ...
          {!otpSent ? (
            <form className="space-y-4" onSubmit={handleSendOtp}>
              ...
              <Button type="submit" className="w-full h-12 bg-ocean-gradient font-semibold text-primary-foreground rounded-xl text-base">Send OTP <ArrowRight className="w-4 h-4 ml-2" /></Button>
            </form>
          ) : (
            <form className="space-y-4" onSubmit={handleVerifyOtp}>
              ...
              <Button type="submit" className="w-full h-12 bg-ocean-gradient font-semibold text-primary-foreground rounded-xl text-base">Verify & Sign In</Button>
